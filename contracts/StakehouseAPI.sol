...
import { IRageQuitAssistant } from "@blockswaplab/stakehouse-contract-interfaces/contracts/interfaces/IRageQuitAssistant.sol";
import { IRageQuitAssistantDeployer } from "@blockswaplab/stakehouse-contract-interfaces/contracts/interfaces/IRageQuitAssistantDeployer.sol";
import { IShanghaiSweepReporting } from "@blockswaplab/stakehouse-contract-interfaces/contracts/interfaces/IShanghaiSweepReporting.sol";
import { IFullWithdrawals } from "@blockswaplab/stakehouse-contract-interfaces/contracts/interfaces/IFullWithdrawals.sol";
...
abstract contract StakehouseAPI {
    ...
    function getStakeHouseRegistry(address _stakeHouse) internal view virtual returns (IStakeHouseRegistry stakehouse) {
        ...
        if(chainId == MainnetConstants.CHAIN_ID || chainId == GoerliConstants.CHAIN_ID) {
    ...
    /// @dev Get the interface connected to an arbitrary rage quit assistant smart contract
    function getRageQuitAssistant(address _assistant) internal view virtual returns (IRageQuitAssistant assistant) {
        uint256 chainId = _getChainId();

        if(chainId == MainnetConstants.CHAIN_ID || chainId == GoerliConstants.CHAIN_ID) {
            assistant = IRageQuitAssistant(_assistant);
        }
    ...
    /// @dev Get the interface connected to the sweep reporter smart contract
    function getRageQuitAssistantDeployer() internal view virtual returns (IRageQuitAssistantDeployer deployer) {
        uint256 chainId = _getChainId();

        if(chainId == MainnetConstants.CHAIN_ID) {
            deployer = IRageQuitAssistantDeployer(MainnetConstants.RageQuitAssistantDeployer);
        }

        else if (chainId == GoerliConstants.CHAIN_ID) {
            deployer = IRageQuitAssistantDeployer(GoerliConstants.RageQuitAssistantDeployer);
        }
    ...
    /// @dev Get the interface connected to the full withdrawals smart contract
    function getFullWithdrawals() internal view virtual returns (IFullWithdrawals withdrawals) {
        uint256 chainId = _getChainId();

        if(chainId == MainnetConstants.CHAIN_ID) {
            withdrawals = IFullWithdrawals(MainnetConstants.FullWithdrawals);
        }

        else if (chainId == GoerliConstants.CHAIN_ID) {
            withdrawals = IFullWithdrawals(GoerliConstants.FullWithdrawals);
        }
    ...
    /// @dev Get the interface connected to the sweep reporter smart contract
    function getSweepReporterAndPartialWithdrawals() internal view virtual returns (IShanghaiSweepReporting shanghai) {
        uint256 chainId = _getChainId();

        if(chainId == MainnetConstants.CHAIN_ID) {
            shanghai = IShanghaiSweepReporting(MainnetConstants.ShanghaiSweepReporter);
        }

        else if (chainId == GoerliConstants.CHAIN_ID) {
            shanghai = IShanghaiSweepReporting(GoerliConstants.ShanghaiSweepReporter);
        }
